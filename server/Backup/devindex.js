...
// ==========================================
// 6. CUSTOMER MANAGEMENT APIs (UPGRADED & FIXED)
// ==========================================

// HELPER: Auto-Save New Location (Smart Logic)
...
// 1. GET ALL CUSTOMERS (No-Crash Version)
app.get('/api/customers', authenticate, async (req, res) => {
    try { 
        ...
        // Calculate Status in JS (Safe from SQL Mode errors)
...
// 2. CREATE CUSTOMER (Fixes 404 Error)
app.post('/api/customers', authenticate, upload.fields([{ name: 'photo' }, { name: 'aadhar' }, { name: 'pan' }]), async (req, res) => {
    const { 
        ...
        drivingLicense, consent, // Mapped to 'wish' column
        nomineeRelOther 
    } = req.body;
    
// 3. SMART LOCATION SEARCH (Pincode OR Name)
app.get('/api/master/locations/search', authenticate, async (req, res) => {
    const { query } = req.query;
    if (!query || query.length < 3) return res.json([]); // Only search if 3+ chars

    try {
        const sql = `
            SELECT * FROM \`Master_Locations\` 
            WHERE \`Pincode\` LIKE ? OR \`Post_Office\` LIKE ? 
            LIMIT 10
        `;
    ...
    try {
        ...
        // Note: We use the 'wish' column to store 'consent' (Yes/No)
...
// 5. CUSTOMER JOURNEY (The 360 View)
app.get('/api/customers/journey/:custId', authenticate, async (req, res) => {
    ...
    try {
        ...
        // B. Sales Events (Vehicle, Insurance, RTO)
        if (sale.length > 0) {
            const s = sale[0];
            timeline.push({ type: 'Purchase', date: s.Date, desc: `Bought Vehicle: ${s['Frame No']}`, color: 'green' });
            
            if (s.Policy_No) {
                timeline.push({ type: 'Insurance', date: s.Policy_Date || s.Date, desc: `Policy: ${s.Policy_No} (${s.Insurance_Co})`, color: 'cyan' });
            }
